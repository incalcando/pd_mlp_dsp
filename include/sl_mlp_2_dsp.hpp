...
// static constexpr std::array<size_t,2> MLP_HIDDEN_LAYERS{15, 15};
...
class Sl_mlp_2
{
    public:
        ...
        void set_data_pair(const  idsp::c_vector<double, MLP_INPUT_SIZE>& input, 
                            const  idsp::c_vector<double, MLP_OUTPUT_SIZE>& target) 
        {
            mlp.accumulate_data(input, target);
        }
        void update_with_data_pair(const  idsp::c_vector<double, MLP_INPUT_SIZE>& input, 
                            const  idsp::c_vector<double, MLP_OUTPUT_SIZE>& target) 
        {
            mlp.accumulate_data(input, target);
            mlp.update(input, target, 0.01);
        }
    ...
    private:
        ...
        // MLP_2LYR<MLP_INPUT_SIZE, MLP_HIDDEN_LAYERS[0], MLP_HIDDEN_LAYERS[1], MLP_OUTPUT_SIZE> mlp;
